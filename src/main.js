...
function updateOutputDeviceLabels() {
  const isWindows = /Win/i.test(navigator.userAgent || navigator.platform);
  if (isWindows) {
    if (outputDeviceName) outputDeviceName.textContent = 'MicMorph (Virtual Cable)';
    if (meetSelectHint) meetSelectHint.innerHTML = '💡 In Google Meet / Zoom, select <strong>"MicMorph"</strong> (or CABLE Output) as your mic.';
  } else {
